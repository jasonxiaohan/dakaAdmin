...
layui.define(['table', 'form','util'], function(exports){
  ...
  table.render({
    ...
    ,cols: [[
      {type: 'checkbox', fixed: 'left'}
      ,{field: 'roleName', title: '岗位名称'}
      ,{field: 'rights', title: '拥有权限'}
      ,{field: 'descr', title: '岗位描述'}
  ...
  //监听工具条
  table.on('tool(LAY-department-back-role)', function(obj){
    var data = obj.data;
    if(obj.event === 'del'){
      ...
      admin.popup({
        ...
        ,id: 'LAY-popup-user-add'
        ,success: function(layero, index){
          view(this.id).render('department/roleform', data).done(function(){
